...
___
Создание MVP-архитектуры для интернет-магазина, ее отображение на UML-схеме, написание полной функциональности сайта и подключение API

https://github.com/user-attachments/assets/bdc4b99f-0b99-48ae-b7d6-0a58f50a8910
...
* Оформление заказа и заполнение форм
...
### Отработанные навыки
* Работа в рамках парадигмы ООП
* Разработка архитектуры приложения
* Объявление типов
* Написание кода компонентов и моделей данных
* Интеграция с API
___
...
При создании архитектуры использовался событийно-ориентированный подход и паттерн MVP. Код состоит из 3-х слоев:
...
Взаимодействие слоев приложения происходит через события и подписки: пользователь инициирует действие, взаимодействуя с интерфейсом, презентер перехватывает действие и обновляет модель. После изменения данных в модели срабатывает событие `emitChanges()`, уведомляющее подписанные компоненты о необходимости обновления. Компоненты отображения получают новые данные и обновляют пользовательский интерфейс.
...
## Модели данных
### Класс `Model<T>`
Абстрактный класс, содержащий конструктор, принимащий данные, и метод `emitChanges()` для регистрации события в `EventEmitter`.
...
* `order` — содержит данные заказа, введенные пользователем
...
### Класс `OrderDelivery`
Отображает модальное окно со способом оплаты и адресом доставки. Данный класс наследуется от класса `Form`.
...
### Класс `OrderContacts`
Отображает модальное окно с контактами пользователя. Данный класс наследуется от класса `Form`.
